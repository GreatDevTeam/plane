...
1. Yjs sync is a merge, never a replace. The browser caches the document in IndexedDB (`packages/editor/src/core/hooks/use-yjs-setup.ts`), so a client that had opened the page before merges the old and the new document and ends up showing both versions — and pushes that merged state back to the server. The title fragment merges the same way, and the live server syncs it back to `pages.name`, so the page name ends up as the old and the new title concatenated.
2. While any editor has the page open, the live server holds the document in memory and stores that state back over `description_binary` **and** `description_html`, silently reverting the change. The live endpoint applies the rewrite to that in-memory document (and pushes the resulting Yjs update to every server holding it, over the Redis admin channel), so open editors pick the change up instead.

There is **no fallback**: clearing `description_binary`/`description_json` and letting the live server rebuild the snapshot from HTML produces exactly the unrelated document described above. When the live server cannot be reached, the update is refused with `503` and the page is left untouched (`PageDetailAPIEndpoint.patch`), so a misconfigured deployment is loud instead of scrambling pages. A page whose `description_binary` is still empty has no snapshot for any client to hold, so it is updated without the live server.

Only the fields that actually change are handed over, and the live server only rewrites the fragments it is given — rewriting the body with the stored HTML while somebody is editing would revert whatever is not saved yet, and the same holds for the title.

The API reaches the live server through `LIVE_BASE_URL` + `LIVE_BASE_PATH` and authenticates with `LIVE_SERVER_SECRET_KEY`. **`LIVE_BASE_URL` has to be set on the API container**, not just on the live one (`deployments/cli/community/docker-compose.yml`, `deployments/aio/community/variables.env`).
